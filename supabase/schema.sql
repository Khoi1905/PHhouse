    ...
    create table if not exists units (
      ...
      pinned_at timestamptz,
      -- "Top phòng" (tên hiển thị tùy chỉnh trong app_settings): null = không
      -- trong danh sách. Độc lập hoàn toàn với pinned_at — 1 phòng vừa ghim vừa
      -- top không ảnh hưởng nhau. Không cascade theo tòa (khác ghim), chỉ set
      -- thủ công từng phòng. Không nhạy cảm — sale xem được.
      top_added_at timestamptz,
      created_at timestamptz default now(),
      updated_at timestamptz default now()
    );
    ...
    -- Safety net for DBs where `units` already existed before top_added_at was
    -- introduced — same reasoning as buildings above.
    alter table units add column if not exists top_added_at timestamptz;
    ...
    -- Generic key-value settings store — hiện chỉ dùng cho tên hiển thị tùy
    -- chỉnh của "Top phòng" (admin đổi tên, danh sách phòng bên trong giữ
    -- nguyên). Không nhạy cảm, sale đọc được, chỉ admin sửa (xem RLS ở mục 5).
    create table if not exists app_settings (
      key text primary key,
      value text
    );
    insert into app_settings (key, value) values ('top_units_label', 'Top phòng')
    ...
    -- app_settings: cả 2 role đọc được (vd tên hiển thị "Top phòng"), chỉ admin sửa.
    ...
    --    owner phone|bank — none of those columns are even referenced here, so
    --    this path cannot leak them regardless of role. commission_sale_pct IS
    --    selected on purpose — same as buildings_sale_view, it's public to sale.
    --
    ...
    --
    --    Ranking has 3 tiers, in this order, WITHIN the already-filtered result
    ...
    create or replace function search_units(
      ...
      p_top_only boolean default false
    )
    returns table (
      id uuid,
      building_id uuid,
    ...
    begin
      ...
      where
        ...
        and (p_top_only is not true or u.top_added_at is not null)
        -- Sale never sees full rooms, enforced regardless of the params above.
        and (v_role = 'admin' or u.status <> 'Full')
      order by
        case
          when u.pinned_at is not null and u.top_added_at is not null then 0
